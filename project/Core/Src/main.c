...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if (GPIO_Pin == GPIO_PIN_13){
	  HAL_GPIO_TogglePin(LDDer_GPIO_Port, LDDer_Pin);
	  HAL_UART_Transmit(&huart2, "PB1\r\n", 5, 10);
  }else if (GPIO_Pin==GPIO_PIN_1){
	  HAL_GPIO_WritePin(LDDer_GPIO_Port, LDDer_Pin, 0);
	  for(uint16_t counter = 0; counter < 0xEFFF; counter ++){}
	  HAL_GPIO_WritePin(LDDer_GPIO_Port, LDDer_Pin, 1);
  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, LDDer_Pin|LDIz_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LDDer_Pin LDIz_Pin */
  GPIO_InitStruct.Pin = LDDer_Pin|LDIz_Pin;
